...
public class Worm {
	...
	public double getMass() {
		...
		return (4 * getDensity() * Math.PI * Math.pow(getRadius(), 3) / 3);			//Om de een of andere reden werkt het niet als 4/3 samen staat?
	}
 ...
 *              | 	then throw new ModelException
	...
	public void jump() {
		...
		setPosition( getX()+delta[0], getY()+delta[1] );			/** @note	Hier stond ...delta[1[, ...delta[2] => in java tellen arrays vanaf 0!
		 															*			Out of boundsexception bij jump is nu weg, maar nog steeds bug in jump.*/
